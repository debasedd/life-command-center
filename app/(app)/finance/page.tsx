  ...
  return (
    <div className="animate-rise">
      <header className="flex items-center justify-between mb-4 pt-1">
        <h1 className="text-[17px] font-semibold tracking-[-0.02em] text-[#f7f8f8]">Keuangan</h1>
        ...
        <>
          <Card className="mb-3">
            <div className="flex gap-1 mb-3 rounded-md bg-white/[0.03] p-1">
              {(["today", "week", "month"] as const).map((r) => (
                <button
                  key={r}
                  onClick={() => setRange(r)}
                  className={`flex-1 rounded py-1 text-[11px] font-medium transition-colors duration-150 ${range === r ? "bg-white/[0.08] text-[#f7f8f8]" : "text-[#8a8f98]"}`}
                >
            ...
            <div className="grid grid-cols-3 gap-2 text-center mb-3">
              <div>
                <p className="text-lg font-semibold text-[#2fbd50] tabular-nums">{idr(income)}</p>
                <p className="text-[10px] text-[#8a8f98]">Masuk</p>
              </div>
              <div>
                <p className="text-lg font-semibold text-[#eb5757] tabular-nums">{idr(expense)}</p>
                <p className="text-[10px] text-[#8a8f98]">Keluar</p>
              </div>
              <div>
                <p className={`text-lg font-semibold tabular-nums ${net >= 0 ? "text-[#f7f8f8]" : "text-[#eb5757]"}`}>{idr(net)}</p>
                <p className="text-[10px] text-[#8a8f98]">Selisih</p>
              </div>
            </div>
            <div className="pt-3 border-t border-white/[0.06]">
              <p className="text-[10px] text-[#8a8f98] mb-2">Pengeluaran per kategori</p>
              {byCat.length === 0 ? (
                <p className="text-xs text-[#4a4d52]">Belum ada data</p>
              ) : (
                    ...
                    <div key={cat.id} className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between text-[11px] mb-1">
                          <span className="text-[#d0d6e0]">{cat.name}</span>
                          <span className="text-[#8a8f98] tabular-nums">{idr(total)}</span>
                        </div>
                        <div className="h-1 rounded-full bg-white/[0.05] overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-300" style={{ width: `${expense ? (total / expense) * 100 : 0}%`, background: cat.color || "#5e6ad2" }} />
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {groupedByDay.length === 0 && (
            <div className="text-center py-14">
              <p className="text-sm text-[#62666d]">Belum ada transaksi di rentang ini</p>
            </div>
          )}
          {groupedByDay.map(([d, items]) => (
            <div key={d} className="mb-3">
              <p className="text-[11px] text-[#62666d] mb-1.5">
                ...
                {items.map((tx) => (
                  <Card key={tx.id} className="!py-2.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-[#62666d] flex items-center gap-1.5">
                        ...
                        <Select
                          ...
                          className="!py-0 !px-1 !text-[10px] !bg-transparent !border-none !w-auto inline-block !text-[#8a8f98]"
                        >
                          {cats.map((c) => (
                    ...
                    <div className="text-right shrink-0 flex items-center gap-1">
                      <div className={`text-sm font-semibold tabular-nums ${tx.type === "INCOME" ? "text-[#2fbd50]" : "text-[#f7f8f8]"}`}>
      ...
      {tab === "goals" && (
        <>
          ...
          {goals.length === 0 && (
            <div className="text-center py-14">
              <p className="text-sm text-[#62666d]">Belum ada target tabungan</p>
            </div>
          )}
          {goals.map((g) => (
            <Card key={g.id} className="mb-2">
              <div className="flex items-center justify-between mb-2.5">
                <div className="font-medium text-[#f7f8f8] flex items-center gap-2">
                  <span>{g.icon}</span> {g.name}
                </div>
                {g.status === "ACHIEVED" && <span className="text-[10px] text-[#27a644] border border-[#27a644]/30 rounded-full px-2 py-0.5 font-medium">Tercapai</span>}
              </div>
              ...
              <div className="flex justify-between text-[11px] text-[#8a8f98] mb-2 tabular-nums">
                <span>{idr(g.currentAmount)} / {idr(g.targetAmount)}</span>
                <span>{Math.round(g.progress * 100)}%</span>
              </div>
              <div className="text-[11px] text-[#62666d] mb-3">
      ...
      <Sheet open={addSheet} onClose={() => setAddSheet(false)} title="Catat Transaksi">
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setType("EXPENSE")} className={`rounded-md py-2.5 font-medium text-sm transition-colors duration-150 ${type === "EXPENSE" ? "bg-[#eb5757] text-white" : "bg-white/[0.03] border border-white/[0.08] text-[#8a8f98]"}`}>Keluar</button>
            <button onClick={() => setType("INCOME")} className={`rounded-md py-2.5 font-medium text-sm transition-colors duration-150 ${type === "INCOME" ? "bg-[#27a644] text-white" : "bg-white/[0.03] border border-white/[0.08] text-[#8a8f98]"}`}>Masuk</button>
          </div>
          <div>
            ...
            <div className="flex gap-1.5 mt-2">
              {[5000, 10000, 20000, 50000, 100000].map((v) => (
                <button key={v} onClick={() => setAmount(String(v))} className="flex-1 rounded-md bg-white/[0.04] border border-white/[0.06] py-1.5 text-[11px] text-[#d0d6e0] card-press">{v / 1000}k</button>
              ))}
            </div>
          </div>
          <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Catatan, mis: nasi padang / ojek ke sekolah" />
          {aiHint && <p className="text-[11px] text-[#7170ff]">{aiHint}</p>}
          <div>
            <label className="text-[11px] text-[#8a8f98] mb-1.5 block">Kategori — kosongkan untuk AI pilih otomatis</label>
            <div className="grid grid-cols-4 gap-1.5">
              {cats.map((c) => (
                <button
                  key={c.id}
                  onClick={() => setCatId(catId === c.id ? "" : c.id)}
                  className={`rounded-md py-2 flex flex-col items-center gap-0.5 border transition-colors duration-150 ${catId === c.id ? "border-[#7170ff] bg-[#7170ff]/10" : "border-white/[0.08] bg-white/[0.02]"}`}
                >
                  <span className="text-base">{c.icon}</span>
                  <span className="text-[9px] text-[#8a8f98] leading-tight text-center px-0.5">{c.name}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {["🎯", "🚲", "📱", "💻", "🛡️", "✈️", "🎓", "🏠"].map((i) => (
                <button key={i} onClick={() => setGIcon(i)} className={`w-11 h-11 rounded-md text-lg flex items-center justify-center border transition-colors duration-150 ${gIcon === i ? "border-[#7170ff] bg-[#7170ff]/10" : "border-white/[0.08] bg-white/[0.02]"}`}>{i}</button>
              ))}
            </div>
          </div>
          <div>
            <label className="text-[11px] text-[#8a8f98] mb-1 block">Deadline (opsional)</label>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-1.5">
            {[20000, 50000, 100000, 200000].map((v) => (
              <button key={v} onClick={() => setDepAmount(String(v))} className="flex-1 rounded-md bg-white/[0.04] border border-white/[0.06] py-2 text-[11px] text-[#d0d6e0] card-press">{v / 1000}k</button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {cats.map((c) => (
            <div key={c.id} className="flex items-center justify-between bg-white/[0.03] border border-white/[0.06] rounded-md px-3 py-2">
              <span className="text-sm text-[#d0d6e0]">{c.icon} {c.name}</span>
              <button
                onClick={async () => {
                  try {
                ...
                className="text-[#62666d] hover:text-[#eb5757] transition-colors duration-150 p-1"
