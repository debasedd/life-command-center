...
const VERDICT_META: Record<string, { color: string; border: string; dot: string }> = {
  AMAN: { color: "text-[color:var(--vr-positive)]", border: "border-[color:var(--vr-positive)]/30", dot: "#609f89" },
  "HATI-HATI": { color: "text-[color:var(--vr-warning)]", border: "border-[color:var(--vr-warning)]/30", dot: "#eab38a" },
  BERISIKO: { color: "text-[color:var(--vr-danger)]", border: "border-[color:var(--vr-danger)]/30", dot: "#f87171" },
};
  ...
  return (
    <div className="animate-rise">
      <header className="mb-5 pt-1">
        <p className="vr-kicker">Simulasi</p>
        <h1 className="vr-display mt-1">What-If Simulator</h1>
            ...
            <div className="flex gap-2">
              {([["Cash lunas", 0], ["Cicil 3×", 3], ["Cicil 6×", 6]] as [string, number][]).map(([label, m]) => (
                <button
                  key={m}
                  onClick={() => setMonths(m)}
                  className={`flex-1 rounded-md py-2 text-[13px] font-medium transition-colors duration-150 ${months === m ? "bg-[color:var(--vr-primary)] text-white" : "bg-white/[0.03] border border-lineSoft text-muted"}`}
                >
                  {label}
                </button>
              ))}
        ...
        <div className="animate-rise space-y-3">
          ...
          <Card>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <div className="text-sm font-semibold text-[color:var(--vr-positive)] tabular-nums">{idr(result.monthlyIncome)}</div>
                <div className="text-[10px] text-muted">masuk</div>
              </div>
              <div>
                <div className="text-sm font-semibold text-[color:var(--vr-danger)] tabular-nums">{idr(result.monthlyExpense)}</div>
                <div className="text-[10px] text-muted">keluar</div>
              </div>
              <div>
                <div className={`text-sm font-semibold tabular-nums ${result.monthlySurplus >= 0 ? "text-ink" : "text-[color:var(--vr-danger)]"}`}>{idr(result.monthlySurplus)}</div>
                ...
                <Card key={i} className="!py-3">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 pr-2">
                      <div className="text-sm font-medium text-ink">{g.icon} {g.name}</div>
                    ...
                    <div className="text-right shrink-0">
                      {g.monthsDelayed !== null ? (
                        <div className={`text-sm font-semibold tabular-nums ${(g.monthsDelayed || 0) > 3 ? "text-[color:var(--vr-danger)]" : (g.monthsDelayed || 0) > 0 ? "text-[color:var(--vr-warning)]" : "text-[color:var(--vr-positive)]"}`}>
          ...
          <Card>
            ...
            <div className="text-center my-4">
              <div className="text-2xl font-semibold text-[color:var(--vr-accent)] tabular-nums">{idr(result.opportunity10y)}</div>
